...
export const columns: ColumnDef<UrlRow>[] = [
  ...
  // ── URL column: fixed small width, truncate long URLs ───────────────────
  {
    accessorKey: "original_url",
    header: ({ column }) => <SortHeader column={column} label="URL" />,
    cell: ({ row }) => {
      const url = row.original.original_url
      const truncate = (s: string, max = 150) =>
        s.length <= max ? s : s.slice(0, 30) + "…" + s.slice(-50)
      return (
  ...
  // ── Title column: fixed small width, truncate long titles ───────────────
  {
    accessorKey: "title",
